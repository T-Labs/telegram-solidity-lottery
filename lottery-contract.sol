...
contract SimpleFairLottery is MyAdvancedToken {
    ...
    function buyTicket(uint ticketNum) returns (bool success)  {
        if ( (ticketNum < 0) || (ticketNum >= countTickets)) return false;  // Check if the number is right
        if (balanceOf[msg.sender] < ticketPrice) return false;              // Check if the sender has enough
        if ( tickets[ticketNum] != 0 ) return false;                        // Check if ticket already buyed
        ...
        tickets[ticketNum] = msg.sender;                                    // write senders address to ticketlist
        return true;
    }
